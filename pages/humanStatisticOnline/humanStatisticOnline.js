// pages/humanStatisticOnline/humanStatisticOnline.js
import {
    StorageKey
} from "../../utils/config"
import {
...
Page({
    ...
    async fetchData() {
        ...
        try {
            ...
            (data || []).forEach((human) => {
                const sampleNumberIndex = sampleNumberMix.findIndex(o => o.sampleNumber === human.sampleNumber);
                if (sampleNumberIndex > -1) {
                    sampleNumberMix[sampleNumberIndex].count += 1
                } else {
                    sampleNumberMix.push({
                        sampleNumber: human.sampleNumber,
                        count: 1
                    })
                }

                const unitIndex = unitSampleNumber.findIndex(o => o.unit === human.unit);
                if (unitIndex > -1) {
                    const sampleNumbers = unitSampleNumber[unitIndex].sampleNumbers
                    const sampleNumberIndex = sampleNumbers.findIndex(p => p.sampleNumber === human.sampleNumber);
                    if (sampleNumberIndex > -1) {
                        sampleNumbers[sampleNumberIndex].objs.push(human)
                    } else {
                        sampleNumbers.push({
                            sampleNumber: human.sampleNumber,
                            objs: [human]
